...
def filter_pipeline(request):
    ...
    assigned_candidate_ids = set(QuizAssignment.objects.filter(
        assigned_to_candidate__isnull=False
    ).values_list('assigned_to_candidate_id', flat=True))
    return render(
        request,
        template,
        {
...
#     """
#     This method renders candidates with their match scores
#     """
#     recruitment_id = request.GET.get("recruitment")

#     # Get candidates with their match analysis
#     candidates = Candidate.objects.filter(is_active=True).select_related(
#         'recruitment_id',
#         'job_position_id'
#     )

#     # Filter by recruitment if specified
#     if recruitment_id:
...
#     # Create a dictionary of candidate match scores
#     candidate_scores = {}
#     for score in match_scores:
#         candidate_scores[score.candidate_id] = {
#             'match_score': float(score.match_score),  # Convert Decimal to float
#             'skills_match_score': float(score.skills_match_score),
#             'experience_years': float(score.experience_years),
#             'qualification': score.highest_qualification,
#             'matched_skills': score.skills_matched
#         }

#     template = "candidate/candidate-ranking.html" if candidates.exists() else "candidate/candidate_empty.html"
...
# def candidate_ranking(request):
#     """
#     This method renders candidates with their match scores, with options to filter
#     top N candidates based on match score
#     """
#     recruitment_id = request.GET.get("recruitment")
...
#         if min_score and match_score < float(min_score):
#             continue
...
#         # Create tuple of (candidate, score) for sorting
#         scored_candidates.append((score.candidate, match_score))

#     # Sort candidates by match score (descending)
#     scored_candidates.sort(key=lambda x: x[1], reverse=True)

#     # Get top N candidates if specified
#     if top_n:
#         try:
#             top_n = int(top_n)
#             scored_candidates = scored_candidates[:top_n]
#         except ValueError:
#             pass  # Invalid top_n value, show all candidates

#     # Extract just the candidates from the sorted list
#     filtered_candidates = [candidate for candidate, _ in scored_candidates]

#     template = "candidate/candidate-ranking.html" if candidates.exists() else "candidate/candidate_matchscore_empty.html"
#     return render(
#         request,
#         template,
#         {
#             "candidates": paginator_qry(filtered_candidates, request.GET.get("page")),
#             "recruitments": recruitments,
#             "candidate_scores": candidate_scores,
#             "filtered_count": len(filtered_candidates) if top_n else None,
#         },
#     )
...
# def candidate_ranking(request):
#     """
#     This method renders candidates with their match scores and quiz scores,
#     with options to filter top N candidates based on match score
#     """
#     recruitment_id = request.GET.get("recruitment")
#     top_n = request.GET.get("top_n")
#     min_score = request.GET.get("min_score")
...
#     # Get quiz scores for candidates
#     quiz_assignments = QuizAssignment.objects.filter(
#         assigned_to_candidate__in=candidates,
#         status='completed'
#     ).select_related('quiz')

#     # Create a dictionary of candidate match scores and quiz scores
#     candidate_scores = {}
#     scored_candidates = []

#     # First, initialize with quiz scores
#     for assignment in quiz_assignments:
#         candidate_id = assignment.assigned_to_candidate_id
#         if candidate_id not in candidate_scores:
#             candidate_scores[candidate_id] = {
#                 'quiz_scores': []
#             }

#         candidate_scores[candidate_id]['quiz_scores'].append({
...
#     # Now add match scores and prepare for sorting
#     for score in match_scores:
...
#         candidate_scores[candidate_id].update({
#             'match_score': match_score,
#             'skills_match_score': float(score.skills_match_score),
#             'experience_years': float(score.experience_years),
#             'qualification': score.highest_qualification,
...
def candidate_ranking(request):
    ...
    # Get quiz assignments for candidates
    quiz_assignments = QuizAssignment.objects.filter(
        assigned_to_candidate__in=candidates
    ).select_related('quiz')
    ...
    # First, initialize with quiz scores and quiz assigned status
    for assignment in quiz_assignments:
        candidate_id = assignment.assigned_to_candidate_id

        # Initialize quiz assigned status as False
        if candidate_id not in candidate_scores:
            candidate_scores[candidate_id] = {
                'quiz_assigned': False,  # Initially assume quiz not assigned
                'quiz_scores': []
            }

        # Mark that a quiz has been assigned to this candidate
        candidate_scores[candidate_id]['quiz_assigned'] = True

        # Add quiz details to the candidate's scores
        candidate_scores[candidate_id]['quiz_scores'].append({
            'quiz_title': assignment.quiz.title,
            'score': assignment.score,
            'passing_score': assignment.quiz.passing_score,
            'passed': assignment.passed,
            'completion_date': assignment.completion_date,
            'status': assignment.status  # Completed or In Progress
        })
    ...
    for score in match_scores:
        ...
        if candidate_id not in candidate_scores:
            candidate_scores[candidate_id] = {
                'quiz_assigned': False,  # Default as not assigned
                'quiz_scores': []
            }
...
# @login_required
# @manager_can_enter("recruitment.view_candidate")
# def get_matchscore_analysis(request):
#     """
#     This method is used to track mails sent along with the status
#     """
#     candidate_id = request.GET["candidate_id"]
#     candidate = Candidate.objects.get(id=candidate_id)
#     return render(request, "candidate/matchscore_analysis.html")

@login_required
@manager_can_enter("recruitment.view_candidate")
def get_matchscore_analysis(request):
    """
    View to display candidate match score analysis
    """
    candidate_id = request.GET.get("candidate_id")
    try:
        candidate = Candidate.objects.get(id=candidate_id)
        # Get the latest match analysis for the candidate
        match_analysis = CandidateMatchAnalysis.objects.filter(
            candidate=candidate
        ).select_related('job').first()

        if not match_analysis:
            messages.error(request, "No match analysis found for this candidate")
            return redirect('candidate/candidate_empty.html')
        print("here is the match score analysis ",match_analysis.analysis_details)
        # Printing match analysis (optional for debugging)

        context = {
            "match_analysis": match_analysis,
            "candidate": candidate,
            "skills_matched": match_analysis.skills_matched,
            "analysis_details": match_analysis.analysis_details,
        }
        return render(request, "candidate/matchscore_analysis.html", context)
    except Candidate.DoesNotExist:
...
@login_required
@manager_can_enter("recruitment.view_candidate")
def get_performace_rec(request):
    """
    This method is used to get candidate performance metrics and skills assessment
    """
    candidate_id = request.GET["candidate_id"]
    candidate = Candidate.objects.get(id=candidate_id)

    # Get overall evaluation scores
    evaluation = CandidateEvaluation.objects.filter(candidate=candidate).first()
    if evaluation:
        skills = CandidateSkillAssessment.objects.filter(candidate=candidate)
        skill_data = []
        for skill in skills:
            skill_info = {
        ...
        print("here is skill data",skill_data)
        # skill_data=[{'skill_name':'C++','proficiency_level':4},{'skill_name':'python','proficiency_level':3},{'skill_name':'Git','proficiency_level':2}]
        # In your view
        for skill in skill_data:  # or however you're handling your skills
            skill['width_percentage'] = skill['proficiency_level'] * 20

        print("here is skill data",skill_data)
        resume_impact_score=evaluation.overall_resume_impact
        catagory_impact={
            "education_impact":evaluation.education_impact,
            "experience_impact":evaluation.experience_impact,
            "skills_impact":evaluation.composite_skills_impact,
            "achievements_impact":evaluation.composite_achievements_impact
        }
        print(catagory_impact)
        subcatagory_impact={
            "education_impact":evaluation.education_impact,
            "experience_impact":evaluation.experience_impact,
            "skills_impact":{ "softskills_imapct":evaluation.soft_skills_impact,"technical_skills_impact":evaluation.technical_skills_impact},
            "achievements_impact":evaluation.composite_achievements_impact
        }
        print(subcatagory_impact)
        return render(request, "candidate/performance.html", {'catagory_impacts':catagory_impact,'subcatagory_impacts':subcatagory_impact,'resume_impact_score':resume_impact_score,'skills':skill_data})
    else:
