...
def json_encode(value):
    """
    Converts a Python object to a JSON string, handling datetime objects
    """
...
@register.filter
def avg_rating(ratings, candidate):
    """
    Calculate average rating for a candidate from their ratings
    ...
    Returns:
        int: Rounded average rating
    """
    if not ratings:
        return 0

    avg = ratings.filter(candidate=candidate).aggregate(Avg('rating'))['rating__avg']
    if avg is None:
        return 0

    # Round to nearest integer since we're using it for radio button comparison
    return round(float(avg))


# templatetags/json_filters.py
...
def safe_json(value):
    """
    Safely converts a Python object to a JSON string for JavaScript.
    Handles datetime objects and other Python-specific types.
    """
    def handler(obj):
        if isinstance(obj, datetime):
            return obj.isoformat()
...
def json_encode(value):
    """
    Safely convert Python data to JSON string for JavaScript
    """
    def handler(obj):
        if isinstance(obj, datetime):
    ...
    try:
        ...
        return json.dumps(cleaned_value, default=handler)
